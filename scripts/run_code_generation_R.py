...
def run_vanilla_prompt_codegen_R(
    samples,
    ...
    # run the prompt code generation
...
def run_manual_prompt_codegen_R(
    samples,
    llm: Union[str, Literal[
    "openai-gpt-35",
    "openai-gpt-4",
...
if __name__ == "__main__":
    ...
    samples = pd.read_csv(f"{REPO_BASE_DIR}/benchmark_datasets/R/coding_tasks.csv")
    ...
    # vanilla prompt
    run_vanilla_prompt_codegen_R(samples.iloc[:2], "azure-gpt-4o-mini")

    # manual prompt
    # run_manual_prompt_codegen_R(samples.iloc[:2], "azure-gpt-4o-mini")

    # auto prompt (dspy required)
    # run_auto_prompt_codegen_R(samples.iloc[:2], "azure-gpt-4o-mini")