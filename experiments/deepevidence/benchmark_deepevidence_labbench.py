...
def litqa2():
    df = pd.read_csv(os.path.join(REPO_BASE_DIR, "benchmarks/LabBench/LitQA2_25.csv"))
    ...
    to_process_indices = [
        5, 6, 10, 11, 12, 13, 14, 15, 19, 21, 23, 24
    ]
    ...
    for index, row in tqdm(df.iterrows(), desc="Processing questions", total=len(df)):
        ...
        outputs.update(row)
        with open(os.path.join(RESULTS_DIR, f"litqa2_{index}.json"), 'w') as f:
